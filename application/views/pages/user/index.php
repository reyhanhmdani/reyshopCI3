		...
		<div class="col-md-10 mx-auto">
			<div class="card mb-3">
				<div class="card-header">
					<span>User</span>
					<a href="<?= base_url('user/create') ?>" class="btn btn-sm btn-secondary">Add</a>
						...
						foreach ($content as $row) : $no++; ?>
							<tr>
								...
								<td>
									<p>
										<img
											src="<?= $row->image ? base_url("images/user/$row->image") :
												base_url("images/user/avatar.png") ?>" alt="" height="50">
										<?= $row->name ?>
									</p>
								</td>
								<td><?= $row->email ?></td>
								<td><?= $row->role ?></td>
								<td><?= $row->is_active ? 'Active' : 'No Active' ?></td>
								<td>
